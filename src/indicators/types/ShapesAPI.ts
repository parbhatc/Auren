...
export class ShapesAPI {
  ...
  private getChart(): ChartAPI | null {
    ...
    let chart: any = null
    if (this.widget.activeChart) {
      const activeChart = this.widget.activeChart()
      if (activeChart) {
        chart = activeChart
      }
    }
    if (!chart && this.widget.chart) {
      const defaultChart = this.widget.chart()
      if (defaultChart) {
        chart = defaultChart
      }
    }
    return chart ? (chart as ChartAPI) : null
  }
  ...
  private async executeOperation(chart: ChartAPI, operation: {
    ...
    switch (type) {
      case 'createShape':
        shapeId = await chart.createShape(...(args as [any, any]))
        break
      case 'createMultipointShape':
        shapeId = await chart.createMultipointShape(...(args as [any, any]))
        break
      case 'createAnchoredShape':
        shapeId = await chart.createAnchoredShape(...(args as [any, any]))
        break
      case 'createExecutionShape':
        await chart.createExecutionShape(...(args as [any, any]))
        break
      default:
