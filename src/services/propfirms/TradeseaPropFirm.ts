...
import { TradeseaTradesClient } from '../tradesea/TradeseaTradesClient'
import { TradeseaTradeHandler } from '../tradesea/TradeseaTradeHandler'
import { PracticeTradeHandler } from '../practice/PracticeTradeHandler'
import { getPracticeStatsData } from '../practice/practiceStatsData'
import {
  getPracticeAccountById,
  getPracticeAccountDisplayTitle,
} from '../../constants/practice'
import { normalizeTradeseaStreamInstrument } from '../tradesea/tradeseaInstrument'
import {
...
} from '../tradesea/tradeseaTradesMessages'
import { TradeseaExecution } from '../tradesea/tradeseaExecutions'
import { debugTradeseaSl, debugTradeseaUpl } from '../tradesea/tradeseaDebug'
import { parseTradeseaPosition, TradeseaPosition } from '../tradesea/tradeseaPositions'
import {
  buildTradeseaWsAccountKeys,
  wsRowMatchesAccount,
} from '../tradesea/tradeseaWsAccountKeys'
import { TradeData } from '../stats/StatsCalculator'
import {
...
} from '../tradesea/tradeseaTradelensStats'
...
export class TradeseaPropFirm extends PropFirmBase {
  ...
  private async runConnectStreamsForSelectedAccount(): Promise<void> {
    ...
    try {
      ...
      const hasTradesWs =
        !this.practiceMode && Boolean(this.chartServices?.streamConfig?.tradesReadOrigin)
  ...
  private applyMdsBootstrapForChartSymbol(): void {
    ...
    if (!this.chartServices?.mds || bootstrapKey === this.lastBootstrapKey) return
  ...
  getRenderChart(symbol: string, timeframe: string, isDark: boolean): ReactElement | null {
    ...
    return React.createElement(TradeseaChart, {
      ...
      tradeseaTradeHandler: this.getHandler(),
  ...
  getHandler(): TradeseaTradeHandler | PracticeTradeHandler {
    if (this.practiceMode && this.practiceTradeHandler) {
