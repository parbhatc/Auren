...
import type { PracticeTradePanelProps, OrderSide } from '../../PracticeTradePanel'
import { DOM_LADDER_LEVELS } from '../../domLadderData'
import {
  buildDomLadder,
  calcDomRowPnl,
  formatDomPrice,
...
import DomLadderRow from '../../DomLadderRow'
import { DomLtpLockButton } from '../../DomLtpLockButton'
import { formatPracticeStatMoney } from '../../PracticeAccountStatsBar'
import { DomActionButtons } from '../DomActionButtons'
import type { TradeseaMarketBook } from '../../../../../services/tradesea/tradeseaMarketBook'

/** Must match DomLadderRow minHeight (22px row + 1px separator). */
const DOM_ROW_HEIGHT_PX = 23

export function PracticeDomTab({
  props,
  book,
  bookTick,
  tickSize,
  ...
  panelUi: PracticeTradePanelSettings
}) {
  ...
  const positionUplFmt =
    positionUpl != null ? formatPracticeStatMoney(positionUpl, { decimals: 2 }) : null
        ...
        <div className="flex items-start justify-between gap-2 rounded-lg border border-amber-500/25 bg-gradient-to-br from-amber-500/10 via-[#0f172a] to-[#0f172a] px-2.5 py-2">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] tabular-nums">
              <span className="text-dom-buy">
                Bid <span className="font-semibold text-[#3fb950]">{bidLabel}</span>
              </span>
              <span className="text-[#475569]">·</span>
              <span className="text-dom-sell">
                Ask <span className="font-semibold text-[#f85149]">{askLabel}</span>
              </span>
            </div>
          </div>
