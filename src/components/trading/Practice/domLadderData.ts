...
export const DOM_LADDER_LEVELS = 60
...
export function formatDomRowPnl(
  ...
  if (isEntry) {
    return { text: '$0', cls: 'text-[#7d8590]', isEntry: true }
  }
  const rounded = Math.round(pnl)
  if (Math.abs(rounded) < 1) {
    return { text: '$0', cls: 'text-[#7d8590]', isEntry: false }
  }
  const text = `$${Math.abs(rounded)}`
  if (rounded > 0) return { text, cls: 'text-dom-buy', isEntry: false }
  return { text, cls: 'text-dom-sell', isEntry: false }
}

const DOM_MAX_ROWS = 280
