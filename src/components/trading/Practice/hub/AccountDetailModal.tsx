...
function StatCard({
  ...
  return (
    ...
    >
      <p
        className={`text-[10px] font-semibold uppercase tracking-wider ${isDark ? "text-slate-500" : "text-slate-500"}`}
      >
        {label}
      </p>
      ...
      {sub ? (
        <p
          className={`mt-0.5 text-[10px] ${isDark ? "text-slate-500" : "text-slate-500"}`}
        >
          {sub}
        </p>
      ) : null}
...
export default function AccountDetailModal({
  ...
  const chartLabel =
    getPracticeMarketDataSettings().accountLabel ||
    account.marketDataAccountLabel ||
    "—";
  ...
  useEffect(() => {
    ...
    void practiceAPI.getStats(account.id).then((res) => {
      ...
      if (stats && typeof stats === "object") {
        if ("winRate" in stats && stats.winRate != null)
          setWinRate(`${Number(stats.winRate).toFixed(1)}%`);
        if ("totalTrades" in stats && stats.totalTrades != null)
          setTotalTrades(stats.totalTrades as number);
      }
    });
    return () => {
      cancelled = true;
  ...
  return (
    <Modal
      ...
      subtitle={
        <span
          className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ring-1 ${statusBadgeClass}`}
        >
          {account.status}
        </span>
      }
      ...
      footer={
        <>
          <button
            type="button"
            onClick={onClose}
            className={`flex-1 ${ghostButtonClass(isDark)}`}
          >
            {t("common.cancel")}
          </button>
          {isTerminal ? (
        ...
        <DetailSection title="Performance" isDark={isDark}>
          <div className="grid grid-cols-2 gap-2">
            ...
            <StatCard
              ...
              isDark={isDark}
            />
            <StatCard label="Win rate" value={winRate ?? "…"} isDark={isDark} />
          </div>
        </DetailSection>
        ...
        <DetailSection title="Account" isDark={isDark}>
          ...
          >
            ...
            <div className="flex justify-between gap-3">
              ...
              <span
                className={`font-medium tabular-nums ${isDark ? "text-slate-200" : "text-slate-800"}`}
              >
            ...
            <div className="flex justify-between gap-3">
              <span
                className={`shrink-0 ${isDark ? "text-slate-500" : "text-slate-500"}`}
              >
                Chart data
              </span>
              <span
