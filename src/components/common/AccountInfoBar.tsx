...
class AccountInfoBar extends Component<AccountInfoBarProps> {
  render() {
    ...
    return (
      <div
        className={`flex items-center justify-between gap-1.5 sm:gap-2 md:gap-3 mb-2 sm:mb-3 ${isDark ? 'bg-slate-800/50' : 'bg-white/50'} rounded-lg px-2 sm:px-3 py-1.5 sm:py-2`}
      >
        ...
        {mll !== undefined && (
          <>
            <div className={`h-6 sm:h-8 w-px ${isDark ? 'bg-slate-700' : 'bg-slate-300'}`} />
            <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 flex-1 min-w-0">
              <div className="text-center sm:text-left">
                <div className={`text-[9px] sm:text-[10px] md:text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  MLL
